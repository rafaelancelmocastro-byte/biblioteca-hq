...
export function AssetsPanel({ seriesList, onSaved }: { seriesList: Series[]; onSaved: () => void | Promise<void> }) {
  ...
  const visiblePublishers = useMemo(() => { const q = publisherSearch.trim().toLocaleLowerCase("pt-BR"); return q ? publishers.filter((publisher) => publisher.toLocaleLowerCase("pt-BR").includes(q)) : publishers; }, [publishers, publisherSearch]);
  ...
  const removeSeriesImage = async () => {
    const series = seriesList.find((item) => item.id === seriesId);
    if (!series) return;
    setBusy(true); setMessage("");
    try { await saveSeriesRecord({ ...series, coverKey: "" }); setSeriesImage(null); setSeriesKey(""); await onSaved(); setMessage(`Capa de ${series.title} removida. O acervo usará a capa de uma edição disponível.`); }
    catch (error) { setMessage(error instanceof Error ? error.message : "Falha ao remover a capa."); }
    finally { setBusy(false); }
  };
  ...
  return <section className="studio-panel mt-5 publisher-manager"><div className="studio-panel-title"><div><span>Organização do acervo</span><h2>Editoras e capas</h2></div><FileImage className="text-amber-400" /></div>
    ...
    <label className="admin-list-search">Buscar editora<input className="admin-field" type="search" placeholder="Digite o nome da editora" value={publisherSearch} onChange={(event) => setPublisherSearch(event.target.value)} /></label>
    <div className="publisher-manager-list" aria-label="Editoras cadastradas">{visiblePublishers.map((publisher) => { const key = assets.find((item) => item.publisher === publisher)?.logo_key || ""; return <button key={publisher} type="button" className={selected === publisher ? "active" : ""} onClick={() => choose(publisher)}>{key && urls[key] ? <img src={urls[key]} alt="" /> : <FileImage />}<span>{publisher}</span></button>; })}</div>
    ...
    <div className="mt-5 border-t border-white/10 pt-5"><h3 className="font-bold mb-3">Arte de coleção ou saga</h3><form onSubmit={saveSeriesImage} className="form-grid"><label>Coleção ou saga<AdminSeriesPicker series={seriesList} value={seriesId} onChange={(id) => { setSeriesId(id); setSeriesImage(null); setSeriesKey(""); }} required /></label><label>Nova arte<input className="admin-field" type="file" accept="image/png,image/jpeg,image/webp" onChange={(event) => setSeriesImage(event.target.files?.[0] || null)} /></label><label className="span-2">Ou escolher uma imagem já enviada<select className="admin-field" value={seriesKey} onChange={(event) => setSeriesKey(event.target.value)}><option value="">Nenhuma</option>{keys.map((key) => <option key={key} value={key}>{key}</option>)}</select></label><p className="span-2 text-xs text-slate-400">Sem capa própria, o agrupamento mostra automaticamente a capa de uma de suas edições.</p><div className="span-2 publisher-manager-actions"><button className="studio-primary" disabled={busy}><UploadCloud /> Salvar arte</button>{seriesList.find((item) => item.id === seriesId)?.coverKey && <button type="button" className="admin-delete-action" disabled={busy} onClick={() => void removeSeriesImage()}><Trash2 /> Remover capa</button>}</div></form></div>
