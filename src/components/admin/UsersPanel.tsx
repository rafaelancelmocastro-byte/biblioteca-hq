...
export function UsersPanel({ section = "all" }: { section?: "all" | "users" | "payments" }) {
  ...
  const [approvalTarget, setApprovalTarget] = useState<AccessProfile | null>(null);
  ...
  return <div className="space-y-5">
    ...
    {section !== "payments" && <section className="studio-panel"><div className="studio-panel-title"><div><span>Controle de acesso</span><h2>Usuários</h2></div><strong>{profiles.length}</strong></div><p className="text-xs text-slate-400 mb-4">Depois que o leitor enviar o comprovante pelo WhatsApp, confira o crédito no seu banco. Encontre o e-mail informado na conversa e aprove a conta aqui. O comprovante sozinho não confirma o pagamento.</p>
      ...
      <div className="user-list">{filtered.map((person) => <article key={person.id} className="user-row"><div><strong>{person.email}</strong><p>{person.role === "master" ? "Proprietário" : person.access_status === "pending_payment" ? "Aguardando conferência do PIX" : person.access_status === "lifetime" ? "Acesso vitalício" : "Bloqueado"}</p></div>{person.role === "master" ? <Shield aria-label="Conta do proprietário" /> : <div className="user-actions"><button type="button" className={person.access_status === "lifetime" && person.is_active ? "user-delete" : "studio-primary"} disabled={!!busyId} onClick={() => person.access_status === "lifetime" && person.is_active ? void setAccess(person, false) : setApprovalTarget(person)}>{person.access_status === "lifetime" && person.is_active ? "Bloquear acesso" : "Aprovar PIX e liberar"}</button><button type="button" className="user-delete" disabled={!!busyId || person.id === selfId} onClick={() => setDeleteTarget(person)}><Trash2 /> Excluir</button></div>}</article>)}{filtered.length === 0 && <p className="user-empty">Nenhum usuário neste filtro.</p>}</div>
    </section>}
    {section !== "users" && settings && <section className="studio-panel"><div className="studio-panel-title"><div><span>Pagamento manual</span><h2>Checkout PIX</h2></div><Users className="text-amber-400" /></div><p className="mb-4 text-sm text-slate-400">Preço normal: R$ 29,99. O valor abaixo é o preço promocional cobrado no PIX.</p><div className="form-grid"><label>Valor promocional vitalício (R$)<input className="admin-field" type="number" step="0.01" min="0.01" value={(settings.lifetime_price_cents / 100).toFixed(2)} onChange={(event) => setSettings({ ...settings, lifetime_price_cents: Math.round(Number(event.target.value) * 100) })} /></label><label>Chave PIX<input className="admin-field" value={settings.pix_key} onChange={(event) => setSettings({ ...settings, pix_key: event.target.value })} /></label><label>Nome do recebedor<input className="admin-field" value={settings.pix_merchant_name} onChange={(event) => setSettings({ ...settings, pix_merchant_name: event.target.value })} /></label><label>Cidade do recebedor<input className="admin-field" value={settings.pix_merchant_city} onChange={(event) => setSettings({ ...settings, pix_merchant_city: event.target.value })} /></label><label className="span-2">WhatsApp com DDI<input className="admin-field" value={settings.whatsapp_number} onChange={(event) => setSettings({ ...settings, whatsapp_number: event.target.value })} /></label></div><button className="studio-primary mt-5" onClick={() => void saveSettings()}>Salvar pagamento</button></section>}
    {approvalTarget && <div className="admin-confirm-backdrop"><div className="admin-confirm-dialog" role="dialog" aria-modal="true" aria-labelledby="approve-pix-title"><h2 id="approve-pix-title">Aprovar pagamento?</h2><p>Confirme que o PIX de <strong>{approvalTarget.email}</strong> entrou na sua conta bancária. O comprovante enviado por WhatsApp, sozinho, não é confirmação de crédito.</p><div><button type="button" onClick={() => setApprovalTarget(null)} disabled={!!busyId}>Cancelar</button><button type="button" className="studio-primary" disabled={!!busyId} onClick={() => { const person = approvalTarget; setApprovalTarget(null); void setAccess(person, true); }}>Confirmei o crédito · liberar acesso</button></div></div></div>}
