...
export const AdminPage: React.FC = () => {
  ...
  return <div className="streaming-page admin-studio space-y-8 sm:space-y-10">
    ...
    {tab === "collections" && <><datalist id="registered-publishers">{[...new Set([...seriesList.map((item) => item.publisher), ...publisherNames])].map((publisher) => <option key={publisher} value={publisher} />)}</datalist><div className="studio-grid collections-grid">
      ...
      <section className="studio-panel catalog-manager"><div className="studio-panel-title"><div><span>Organização</span><h2>Coleções e sagas cadastradas</h2></div><strong>{filteredSeries.length}</strong></div><label className="admin-list-search">Buscar coleção, saga ou editora<input className={fieldClass} type="search" placeholder="Digite para filtrar rapidamente" value={collectionSearch} onChange={(event) => setCollectionSearch(event.target.value)} /></label><div className="collection-list">{filteredSeries.map((series) => <div key={series.id} className="admin-series-row"><button type="button" onClick={() => editSeries(series)}><div><strong>{series.title}</strong><span>{series.bannerTone === "saga" ? "Saga" : series.bannerTone === "phase" ? "Fase" : series.bannerTone === "one_shot" ? "Obra fechada" : "Coleção"} · {series.publisher}{series.parentSeriesId ? ` → ${seriesList.find((item) => item.id === series.parentSeriesId)?.title || "Coleção"}` : ""} · {series.startYear}</span><small>{allComics.filter((comic) => comic.seriesId === series.id).length} edições</small></div><Edit3 /></button><button type="button" className="admin-delete-action" onClick={() => setDeleteTarget({ type: "series", id: series.id })}>Excluir</button></div>)}</div></section>
    </div></>}
