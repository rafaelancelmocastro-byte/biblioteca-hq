...
import { AlertCircle, BookCopy, CheckCircle2, Cloud, Database, Edit3, FileImage, FileUp, LibraryBig, Plus, Save, Search, Shield, UploadCloud, X } from "lucide-react";
...
import { AdminSeriesPicker } from "../../components/admin/AdminSeriesPicker";
...
export const AdminPage: React.FC = () => {
  ...
  const [managerIssue, setManagerIssue] = useState("all");
  ...
  const managedComics = useMemo(() => allComics.filter((comic) => {
    ...
    const issueOk = managerIssue === "all" || (managerIssue === "missing_cover" && !comic.coverUrl) || (managerIssue === "missing_synopsis" && !comic.synopsis.trim()) || (managerIssue === "unassigned" && !comic.seriesId) || (managerIssue === "storage_error" && storageStatuses[comic.id] === "error");
  ...
  }).sort((a, b) => managerSort === "az" ? a.title.localeCompare(b.title, "pt-BR") : managerSort === "za" ? b.title.localeCompare(a.title, "pt-BR") : managerSort === "size" ? b.fileSizeMb - a.fileSizeMb : b.addedAt.localeCompare(a.addedAt)), [allComics, debouncedSearch, managerFormat, managerPublisher, managerSeries, managerYear, managerStorage, managerSort, managerIssue, storageStatuses]);
  ...
  const filteredSeries = useMemo(() => seriesList.filter((series) => !collectionNeedle || seriesPath(series, seriesList).toLocaleLowerCase("pt-BR").includes(collectionNeedle)), [seriesList, collectionNeedle]);
  const orderedSeries = useMemo(() => [...filteredSeries].sort((a, b) => { const ar = a.parentSeriesId ? 1 : 0; const br = b.parentSeriesId ? 1 : 0; if (ar !== br) return ar - br; const ap = a.parentSeriesId ? seriesList.find((item) => item.id === a.parentSeriesId)?.title || "" : a.title; const bp = b.parentSeriesId ? seriesList.find((item) => item.id === b.parentSeriesId)?.title || "" : b.title; return ap.localeCompare(bp, "pt-BR") || a.title.localeCompare(b.title, "pt-BR"); }), [filteredSeries, seriesList]);
  ...
  return <div className="streaming-page admin-studio space-y-8 sm:space-y-10">
    <header className="admin-editorial-header px-1 pt-1"><span>Central do proprietário</span><div><div><h1>Configurações</h1><p>Gerencie publicações, estrutura do acervo, usuários e armazenamento.</p></div><div className="admin-header-metrics"><span><strong>{allComics.length}</strong> publicações</span><span><strong>{seriesList.length}</strong> agrupamentos</span><span><strong>{formatFileSize(totalMb)}</strong> em arquivos</span></div></div></header>
    <div className="admin-global-search"><Search aria-hidden="true" /><input type="search" placeholder="Buscar publicação, coleção, editora, autor ou arquivo..." value={adminSearch} onChange={(event) => setAdminSearch(event.target.value)} />{adminSearch && <button type="button" aria-label="Limpar busca" onClick={() => setAdminSearch("")}><X /></button>}{adminSearch.trim().length >= 2 && <div className="admin-global-results">{adminSearchResults.comics.length > 0 && <section><span>Publicações</span>{adminSearchResults.comics.map((comic) => <button key={comic.id} type="button" onClick={() => { setAdminSearch(""); startEditing(comic); }}><strong>{comic.title}</strong><small>{comic.seriesTitle} · #{comic.issueNumber}</small></button>)}</section>}{adminSearchResults.series.length > 0 && <section><span>Coleções & sagas</span>{adminSearchResults.series.map((series) => <button key={series.id} type="button" onClick={() => { setAdminSearch(""); setTab("collections"); editSeries(series); }}><strong>{series.title}</strong><small>{series.publisher}</small></button>)}</section>}{adminSearchResults.publishers.length > 0 && <section><span>Editoras</span>{adminSearchResults.publishers.map((publisher) => <button key={publisher} type="button" onClick={() => { setAdminSearch(""); setTab("publishers"); }}><strong>{publisher}</strong><small>Abrir gerenciador de editoras</small></button>)}</section>}{!adminSearchResults.comics.length && !adminSearchResults.series.length && !adminSearchResults.publishers.length && <p>Nenhum resultado encontrado.</p>}</div>}</div>
    <nav className="studio-tabs" aria-label="Seções de configuração"><button className={tab === "overview" ? "active" : ""} onClick={() => setTab("overview")}>Visão geral</button><button className={tab === "catalog" ? "active" : ""} onClick={() => setTab("catalog")}><LibraryBig /> Publicações</button><button className={tab === "import" ? "active" : ""} onClick={() => setTab("import")}><UploadCloud /> Importar</button><button className={tab === "collections" ? "active" : ""} onClick={() => setTab("collections")}><BookCopy /> Coleções & sagas</button><button className={tab === "publishers" ? "active" : ""} onClick={() => setTab("publishers")}>Editoras</button><button className={tab === "users" ? "active" : ""} onClick={() => setTab("users")}><Shield /> Usuários</button><button className={tab === "payments" ? "active" : ""} onClick={() => setTab("payments")}>Pagamentos</button><button className={tab === "status" ? "active" : ""} onClick={() => setTab("status")}><Cloud /> Sistema</button></nav>
    {(tab === "import" || tab === "collections" || tab === "publishers") && <details className="catalog-organization-guide"><summary>Como organizar editora, coleção, saga, volume e edição</summary><div><p><strong>Editora ou selo</strong> publica a HQ, como DC Comics ou Marvel. Você informa a editora ao criar o agrupamento.</p><p><strong>Coleção</strong> reúne um título contínuo ou uma franquia de leitura, como Action Comics ou Superman. Cadastre-a uma vez e associe suas edições e obras fechadas.</p><p><strong>Saga</strong> é um arco ou evento com começo e fim, como uma história que pode aparecer em várias coleções. Vincule a saga à coleção principal. Se o evento atravessar coleções, escolha a coleção mais representativa para navegação e informe as demais em Categorias / tags.</p><p><strong>Obra fechada</strong> é um encadernado ou minissérie completa em um volume. Cadastre-a como filha da coleção do personagem e associe o PDF como uma edição única.</p><p><strong>Graphic novels de autores independentes</strong>: use a editora real da edição (não o nome do autor). Dentro dela, crie “Graphic Novels” como coleção; para cada livro avulso, crie uma obra fechada filha com o título da obra. Preencha o autor em Roteiro e use uma tag com o nome dele para encontrar suas obras em editoras diferentes.</p><p><strong>Volume</strong> identifica um tomo, encadernado ou fase quando essa divisão existe na publicação. Use apenas quando o material indicar um volume.</p><p><strong>Edição</strong> é a unidade publicada no arquivo, com número, ano, páginas e capa próprios. Para um livro único, use edição 1. Exemplo: Marvel → Vingadores → Dinastia Kang → edição 1.</p><p>Ordem sugerida: crie a coleção, selecione os PDFs, aplique os dados comuns ao lote e revise número, volume, páginas e capa de cada arquivo.</p></div></details>}
    ...
    {(tab === "catalog" || tab === "import" || tab === "edit") && <div className="studio-grid">
      ...
      {(tab === "import" || tab === "edit") && <form className="studio-panel comic-editor" onSubmit={submitComic}>
        ...
        <div className="form-grid">
          <label className="span-2">Título{tab === "import" && batchPdfs.length > 0 && <small>Gerado pelo nome de cada arquivo na publicação em lote</small>}<input className={fieldClass} value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} required={tab === "edit" || batchPdfs.length === 0} disabled={tab === "import" && batchPdfs.length > 0} /></label>
          <label>Coleção / saga principal<AdminSeriesPicker series={seriesList} value={form.seriesId} onChange={(seriesId) => setForm({ ...form, seriesId })} required /></label>
          ...
          <details className="admin-advanced-fields span-2">
            <summary>Detalhes editoriais e créditos</summary>
            <div className="form-grid">
        ...
        {tab === "import" && (pdf || batchPdfs.length > 0) && <p className="mobile-upload-help" role="status">{draftSaveState === "saving" ? "Salvando arquivos neste dispositivo..." : draftSaveState === "saved" ? "Fila salva neste dispositivo. Você pode sair e voltar para continuar." : draftSaveState === "error" ? "O armazenamento local falhou; não saia desta tela antes de publicar." : "Preparando rascunho..."}</p>} {tab === "import" && mobilePdfPicker && "showOpenFilePicker" in window && <button type="button" className="upload-clear" onClick={() => void choosePdfWithSystemPicker()}>Abrir seletor alternativo de arquivos</button>}
        {tab === "import" && mobilePdfPicker && <p className="mobile-upload-help">A seleção abre em uma tela leve e volta para esta fila. Você também pode abrir o arquivo em Arquivos e usar Compartilhar → Biblioteca HQ.</p>}
        {tab === "import" && (pdf || batchPdfs.length > 0) && <button type="button" className="upload-clear" onClick={() => { setPdf(null); setBatchPdfs([]); setNotice("Seleção de arquivos limpa."); }}>Limpar arquivos selecionados</button>}
        {tab === "import" && pdf && !editing && <button type="button" onClick={() => { setBatchPdfs([pdf]); setPdf(null); }}>Revisar como lote (opções para duplicados)</button>}
      ...
      {tab === "catalog" && <section className="studio-panel catalog-manager">
        ...
        <div className="manager-controls">
          ...
          <label>Coleção<AdminSeriesPicker series={seriesList} value={managerSeries === "all" ? "" : managerSeries} onChange={(id) => setManagerSeries(id || "all")} placeholder="Filtrar por coleção..." /></label>
          <label>Pendência<select className={fieldClass} value={managerIssue} onChange={(event) => setManagerIssue(event.target.value)}><option value="all">Todas</option><option value="missing_cover">Sem capa</option><option value="missing_synopsis">Sem sinopse</option><option value="storage_error">Erro de arquivo</option></select></label><label>Arquivo<select className={fieldClass} value={managerStorage} onChange={(event) => setManagerStorage(event.target.value)}><option value="all">Todos</option><option value="present">Arquivo disponível</option><option value="pending">Pendente</option><option value="error">Erro de sincronização</option></select></label>
    ...
    {tab === "publishers" && <AssetsPanel seriesList={seriesList} onSaved={async () => { await Promise.all([reloadData(true), refreshPublisherNames()]); }} />}
    {tab === "collections" && <><datalist id="registered-publishers">{[...new Set([...seriesList.map((item) => item.publisher), ...publisherNames])].map((publisher) => <option key={publisher} value={publisher} />)}</datalist><div className="studio-grid collections-grid">
      ...
      <section className="studio-panel catalog-manager"><div className="studio-panel-title"><div><span>Organização</span><h2>Coleções e sagas cadastradas</h2></div><strong>{filteredSeries.length}</strong></div><label className="admin-list-search">Buscar coleção, saga ou editora<input className={fieldClass} type="search" placeholder="Digite para filtrar rapidamente" value={collectionSearch} onChange={(event) => setCollectionSearch(event.target.value)} /></label><div className="collection-list">{orderedSeries.map((series) => <div key={series.id} className={`admin-series-row ${series.parentSeriesId ? "child" : "root"}`}><button type="button" onClick={() => editSeries(series)}><div><strong>{series.title}</strong><span>{series.bannerTone === "saga" ? "Saga" : series.bannerTone === "phase" ? "Fase" : series.bannerTone === "one_shot" ? "Obra fechada" : "Coleção"} · {series.publisher}{series.parentSeriesId ? ` → ${seriesList.find((item) => item.id === series.parentSeriesId)?.title || "Coleção"}` : ""} · {series.startYear}</span><small>{allComics.filter((comic) => comic.seriesId === series.id).length} edições</small></div><Edit3 /></button><button type="button" className="admin-delete-action" onClick={() => setDeleteTarget({ type: "series", id: series.id })}>Excluir</button></div>)}</div></section>
    </div></>}
    {tab === "users" && <UsersPanel section="users" />}
    {tab === "payments" && <UsersPanel section="payments" />}
    {tab === "status" && <section className="admin-status-grid"><button onClick={() => setTab("catalog")}><Database /><span><strong>{allComics.length}</strong> Publicações</span></button><button onClick={() => { setManagerStorage("present"); setTab("catalog"); }}><Cloud /><span><strong>{allComics.filter((comic) => storageStatuses[comic.id] === "present").length}</strong> Arquivos disponíveis</span></button><button onClick={() => { setManagerStorage("pending"); setTab("catalog"); }}><Cloud /><span><strong>{allComics.filter((comic) => storageStatuses[comic.id] === "pending").length}</strong> Pendentes</span></button><button onClick={() => { setManagerIssue("storage_error"); setTab("catalog"); }}><AlertCircle /><span><strong>{storageErrorCount}</strong> Erros de arquivo</span></button><button onClick={() => { setManagerIssue("missing_cover"); setTab("catalog"); }}><FileImage /><span><strong>{missingCoverCount}</strong> Sem capa</span></button><article><LibraryBig /><span><strong>{totalPages.toLocaleString("pt-BR")}</strong> Páginas cadastradas</span></article></section>}
