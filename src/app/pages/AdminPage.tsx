...
export const AdminPage: React.FC = () => {
  ...
  const [seriesForm, setSeriesForm] = useState({ id: "", kind: "collection", title: "", publisher: "Marvel", startYear: String(new Date().getFullYear()), endYear: "", expected: "", description: "" });
  ...
  const editSeries = (series?: Series) => setSeriesForm(series ? { id: series.id, kind: series.bannerTone === "saga" ? "saga" : "collection", title: series.title, publisher: series.publisher, startYear: String(series.startYear), endYear: series.endYear ? String(series.endYear) : "", expected: series.totalIssuesExpected ? String(series.totalIssuesExpected) : "", description: series.description } : { id: "", kind: "collection", title: "", publisher: "Marvel", startYear: String(new Date().getFullYear()), endYear: "", expected: "", description: "" });
  ...
  return <div className="streaming-page admin-studio">
    ...
    {tab === "collections" && <div className="studio-grid collections-grid">
      ...
      <section className="studio-panel catalog-manager"><div className="studio-panel-title"><div><span>Organização</span><h2>Coleções e sagas cadastradas</h2></div><strong>{seriesList.length}</strong></div><div className="collection-list">{seriesList.map((series) => <button key={series.id} onClick={() => editSeries(series)}><div><strong>{series.title}</strong><span>{series.bannerTone === "saga" ? "Saga" : "Coleção"} · {series.publisher} · {series.startYear}</span><small>{allComics.filter((comic) => comic.seriesId === series.id).length} edições</small></div><Edit3 /></button>)}</div></section>
    </div>}
