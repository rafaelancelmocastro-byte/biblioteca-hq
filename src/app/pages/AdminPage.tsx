...
export const AdminPage: React.FC = () => {
  ...
  const submitComic = async (event: React.FormEvent) => {
    ...
    if (!series || !form.title.trim() || (!editing && !pdf) || !form.issue || !form.year || !form.pages) { setNotice("Complete título, edição, ano, páginas e PDF antes de publicar."); return; }
    if (pdf && pdf.size > 5 * 1024 * 1024 * 1024) { setNotice("Cada arquivo pode ter no máximo 5 GB no envio direto ao R2."); return; }
    setBusy(true);
    try {
      if (!editing) {
        ...
        if (check.code !== "UNIQUE") { setNotice(`${check.message} Use “Revisar como lote” para comparar e escolher entre manter, substituir ou cancelar.`); return; }
      }
  ...
  const editSeries = (series?: Series) => setSeriesForm(series ? { id: series.id, kind: series.bannerTone === "saga" ? "saga" : "collection", title: series.title, publisher: series.publisher, startYear: String(series.startYear), endYear: series.endYear ? String(series.endYear) : "", expected: series.totalIssuesExpected ? String(series.totalIssuesExpected) : "", description: series.description } : { id: "", kind: "collection", title: "", publisher: "", startYear: "", endYear: "", expected: "", description: "" });
  const submitSeries = async (event: React.FormEvent) => {
    event.preventDefault(); setBusy(true); setNotice("Salvando coleção...");
    try {
      const savedSeriesId = await saveSeriesRecord({ id: seriesForm.id || undefined, title: seriesForm.title, publisher: seriesForm.publisher, startYear: Number(seriesForm.startYear), endYear: seriesForm.endYear ? Number(seriesForm.endYear) : undefined, totalIssuesExpected: seriesForm.expected ? Number(seriesForm.expected) : undefined, description: seriesForm.description, bannerTone: seriesForm.kind });
      await reloadData(true); setForm((current) => ({ ...current, seriesId: savedSeriesId })); editSeries(); setNotice("Coleção salva e disponível no catálogo, mesmo antes de receber edições.");
  ...
  const confirmDelete = async (deleteContents = false) => {
    ...
    try {
      if (deleteTarget.type === "comics") {
        ...
        setNotice(`${count} edição(ões) movida(s) para exclusão reversível.`);
        setSelectedIds([]);
      } else {
        await deleteSeriesRecord(deleteTarget.id, deleteContents);
        setNotice(deleteContents ? "Agrupamento e edições vinculadas excluídos logicamente." : "Agrupamento excluído; edições mantidas no acervo sem coleção.");
      }
