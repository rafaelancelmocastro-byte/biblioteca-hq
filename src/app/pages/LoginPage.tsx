...
export const LoginPage: React.FC<{ onSuccess: () => void }> = ({ onSuccess }) => {
  ...
  const [isRecoveryLink, setIsRecoveryLink] = useState(() => /type=(invite|recovery)/.test(window.location.hash + window.location.search));
  ...
  useEffect(() => {
    ...
    const { data: listener } = supabase.auth.onAuthStateChange((event, session) => {
      if (event === "PASSWORD_RECOVERY") { recoveryRef.current = true; setIsRecoveryLink(true); return; }
      if (event === "SIGNED_IN" && session && !recoveryRef.current && !initialLinkError()) finishLogin();
    });
    void supabase.auth.getSession().then(({ data }) => {
      if (active && data.session && !recoveryRef.current && !initialLinkError()) finishLogin();
    });
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      if (isRecoveryLink) {
        ...
        if (failure) throw failure;
        window.history.replaceState({}, "", "/login"); finishLogin(); return;
      }
      if (mode === "recovery") {
        const { error: failure } = await supabase.auth.resetPasswordForEmail(email.trim(), { redirectTo: AUTH_REDIRECT_URL });
        if (failure) throw failure;
      ...
      if (failure) throw failure;
      finishLogin();
    } catch (failure) { const message = failure instanceof Error ? failure.message : "Não foi possível concluir. Tente novamente."; setError(/Email not confirmed/i.test(message) ? "Confirme seu e-mail antes de entrar. Se o link venceu, reenvie a confirmação abaixo." : message); }
    finally { setBusy(false); }
  };
  ...
  return <div className="login-screen min-h-screen flex items-center justify-center p-4 relative overflow-hidden">
    ...
    <div className="login-card relative w-full max-w-md rounded-[2rem] p-6 sm:p-8">
      ...
      {(mode === "signup" || linkError || /Confirme seu e-mail/.test(error)) && !isRecoveryLink && <button type="button" className="mt-4 w-full text-center text-xs text-amber-300 underline underline-offset-4 disabled:opacity-50" onClick={() => void resendConfirmation()} disabled={busy}>Reenviar e-mail de confirmação</button>}
