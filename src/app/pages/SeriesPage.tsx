...
export const SeriesPage: React.FC<{ onOpenReader: (id: string) => void }> = ({ onOpenReader }) => {
  ...
  const groups = useMemo(() => seriesList.filter((series) => series.publisher === publisher && (kind === "all" || (series.bannerTone === "saga" ? "saga" : "collection") === kind) && series.title.toLocaleLowerCase("pt-BR").includes(groupSearch.toLocaleLowerCase("pt-BR"))), [seriesList, publisher, kind, groupSearch]);
