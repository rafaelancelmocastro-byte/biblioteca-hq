...
export function CheckoutPage({ email, onBack }: { email: string; onBack: () => void }) {
  ...
  return <div className="min-h-dvh bg-[#0f0f11] text-white px-4 py-8 flex justify-center items-start"><div className="w-full max-w-xl rounded-3xl border border-white/10 bg-[#1d1714] p-5 sm:p-8 shadow-2xl">
    ...
    <p className="text-slate-300 text-sm mt-3">Pagamento único de <strong className="text-white">{settings ? (settings.lifetime_price_cents / 100).toLocaleString("pt-BR", { style: "currency", currency: "BRL" }) : "R$ 29,99"}</strong>. Após o envio do comprovante, seu acesso será liberado após a conferência do pagamento.</p>
