...
export default function App() {
  ...
  const { session, profile, offlineUserId, isLoading, signOut, isSupabaseConfigured } = useAuth();
  ...
  const effectiveUserId = session?.user.id || offlineUserId;
  const hasOfflineIdentity = !navigator.onLine && !!offlineUserId && !!profile;
  ...
  if (isSupabaseConfigured && (session || hasOfflineIdentity) && !profile) return <div className="login-screen min-h-dvh flex items-center justify-center p-4"><div className="login-card max-w-md rounded-3xl p-6 text-center"><h1 className="text-xl font-bold">Não foi possível verificar seu acesso</h1><p className="mt-3 text-sm text-slate-300">Confira sua conexão e tente novamente. Nenhum pagamento é necessário enquanto esta verificação estiver indisponível.</p><button className="studio-primary mt-5" onClick={() => window.location.reload()}>Tentar novamente</button></div></div>;
  ...
  if (activeRoute === "/ler" && comicId) {
    return (
      <React.Suspense fallback={<div className="min-h-screen bg-[#05090f] flex items-center justify-center"><div className="w-10 h-10 border-2 border-[#526d8a] border-t-transparent rounded-full animate-spin" /></div>}>
        <ReaderPage comicId={comicId} onBack={backFromReader} onOpenReader={openReader} fallbackUserId={effectiveUserId} />
      </React.Suspense>
    );
  }
